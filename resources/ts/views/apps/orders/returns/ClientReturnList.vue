...
const STORAGE_KEY = 'client-returns-visible-columns'
...
// 👉 Fetching Returns
const { data: returnsData, execute: fetchReturns, isFetching } = await useApi<any>(createUrl('/client-returns', {
  query: {
...
const submitApproval = async () => {
  ...
  const url = `/client-returns/${selectedReturn.value.id}/${approvalAction.value}`
...
const updateStatus = async (id: number, status: string) => {
  processingAction.value = true
  const { error } = await useApi(`/client-returns/${id}`).patch({ status }).json()
...
const exportReturns = async () => {
  ...
  window.open(`/api/client-returns/export?${queryParams}&token=${token}`, '_blank')
}
</script>
...
<template>
  <section>
      ...
      >
        ...
        <template #item.actions="{ item }: { item: any }">
          <div class="d-flex gap-1 align-center">
            <IconBtn v-if="can('client-return.view' as any, 'all' as any)" size="small" @click="viewDetails(item.id)">
              <VIcon icon="tabler-eye" />
              <VTooltip activator="parent">View Details</VTooltip>
            </IconBtn>
      
            <!-- Invoice -->
            <VBtn v-if="can('client-return.view' as any, 'all' as any)" size="small" icon color="primary" variant="tonal" class="rounded" @click="printInvoice(item.id)">
            ...
            <template v-if="item.approval_status === 'PENDING'">
              <IconBtn v-if="can('client-return.approve' as any, 'all' as any)" size="small" color="success" @click="openApprovalDialog(item.id, 'approve')">
                <VIcon icon="tabler-check" />
                <VTooltip activator="parent">Approve</VTooltip>
              </IconBtn>
              <IconBtn v-if="can('client-return.reject' as any, 'all' as any)" size="small" color="error" @click="openApprovalDialog(item.id, 'reject')">
                <VIcon icon="tabler-x" />
    ...
    <VDialog v-model="isDetailsDialogVisible" max-width="800">
      <VCard :title="`Client Return Details - #${selectedReturn?.id}`">
        <VCardText>
          <VRow>
            <VCol cols="12" md="6">
              <div class="text-subtitle-2 mb-1">Client</div>
              <div class="text-body-1 font-weight-bold">{{ selectedReturn?.client?.name || '-' }}</div>
            </VCol>
            <VCol cols="12" md="6">
    ...
    <VDialog v-model="isApprovalDialogVisible" max-width="500">
      <VCard :title="approvalAction === 'approve' ? 'Approve Client Return' : 'Reject Client Return'">
        <VCardText>
          <p>Are you sure you want to {{ approvalAction }} client return #{{ selectedReturn?.id }}?</p>
          <AppTextarea
