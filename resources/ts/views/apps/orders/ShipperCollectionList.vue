...
const { data: collectionsData, execute: fetchCollections, isFetching } = await useApi<any>(createUrl('/shipper-collections', {
  query: {
...
const updateStatus = async (id: number, status: string) => {
  processingAction.value = true
  const { error } = await useApi(`/shipper-collections/${id}`).patch({ status }).json()
...
const exportCollections = async () => {
  ...
  window.open(`/api/shipper-collections/export?${queryParams}&token=${token}`, '_blank')
}
</script>
  ...
  <section>
    ...
    <VCard>
      ...
      >
        ...
        <template #item.collection_date="{ item }: { item: any }">
          <span class="text-sm">{{ new Date(item.collection_date).toLocaleDateString() }}</span>
        </template>
        ...
        <template #item.actions="{ item }: { item: any }">
          <div class="d-flex gap-1 align-center">
            <!-- View Details -->
            <IconBtn v-if="can('shipper-collection.view' as any, 'all' as any)" size="small" @click="viewDetails(item.id)">
              <VIcon icon="tabler-eye" />
              <VTooltip activator="parent">View Details</VTooltip>
            </IconBtn>
      
            <!-- Print Invoice -->
            <VBtn v-if="can('shipper-collection.view' as any, 'all' as any)" size="small" icon color="primary" variant="tonal" class="rounded" @click="printInvoice(item.id)">
              <VIcon icon="tabler-file-invoice" size="20" />
              <VTooltip activator="parent">Print Invoice</VTooltip>
            </VBtn>
      
            <!-- Approval Status (Approve/Reject) -->
            <template v-if="item.approval_status && item.approval_status.toString().toUpperCase() === 'PENDING'">
              <IconBtn v-if="can('shipper-collection.approve' as any, 'all' as any)" size="small" color="success" @click="openApprovalDialog(item.id, 'approve')">
                <VIcon icon="tabler-check" />
                <VTooltip activator="parent">Approve (Approval)</VTooltip>
              </IconBtn>
              <IconBtn v-if="can('shipper-collection.reject' as any, 'all' as any)" size="small" color="error" @click="openApprovalDialog(item.id, 'reject')">
                <VIcon icon="tabler-x" />
            ...
            <template v-if="item.status === 'PENDING' && item.approval_status === 'APPROVED'">
              <VDivider v-if="can('shipper-collection.update' as any, 'all' as any)" vertical class="mx-1" />
              <VBtn 
    ...
    <VDialog v-model="isDetailsDialogVisible" max-width="800">
      <VCard :title="`Collection Details - #${selectedCollection?.id}`">
        <VCardText>
          <VRow>
            <VCol cols="12" md="6">
              ...
              <div class="text-body-1 font-weight-bold">{{ selectedCollection?.total_amount }} EGP</div>
            </VCol>
            <VCol cols="12" md="6">
              <div class="text-subtitle-2 mb-1">Net Amount</div>
              <div class="text-body-1 font-weight-bold text-success">{{ selectedCollection?.net_amount }} EGP</div>
            </VCol>
          </VRow>
        ...
        <VCardActions>
          <VBtn 
            v-if="selectedCollection?.approval_status && selectedCollection.approval_status.toString().toUpperCase() === 'PENDING'"
            color="success" 
            variant="tonal" 
    ...
    <VDialog v-model="isApprovalDialogVisible" max-width="500">
      <VCard :title="approvalAction === 'approve' ? 'Approve Collection' : 'Reject Collection'">
        <VCardText>
          <p>Are you sure you want to {{ approvalAction }} collection #{{ selectedCollection?.id }}?</p>
          <AppTextarea
