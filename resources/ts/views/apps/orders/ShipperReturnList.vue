        ...
        <template #item.actions="{ item }: { item: any }">
          <div class="d-flex gap-1 align-center">
            ...
            <template v-if="item.status === 'PENDING' && item.approval_status === 'APPROVED'">
              <VDivider v-if="can('shipper-return.update' as any, 'all' as any)" vertical class="mx-1" />
              <VBtn v-if="can('shipper-return.update' as any, 'all' as any)" size="x-small" color="success" variant="elevated" :loading="processingAction" @click="updateStatus(item.id, 'COMPLETED')">
                Complete
              </VBtn>
              <VBtn v-if="can('shipper-return.update' as any, 'all' as any)" size="x-small" color="error" variant="outlined" :loading="processingAction" @click="updateStatus(item.id, 'CANCELLED')">
                Cancel
              </VBtn>
            </template>
          </div>
